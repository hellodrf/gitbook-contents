...
* 肌胃（ventriculus）：鸟类的第二个胃，无分泌腺，肌肉发达，用于磨碎食物，以及形成食丸。一些鸟类在肌胃中储存砂砾，协助磨碎食物。
...
* 裸区（apterium）：鸟类皮肤上不着生或几乎不着生羽毛的区域。
...
* 砂囊（gizzard）：同肌胃。
* 生境（habitat）：通称“栖息地”。
* 食丸（pellet）：又称“食团”、“食茧”，鸟类将食物不能消化的部分，如角质、骨骼、毛发，在肌胃中挤压成的团。鸟类必须吐出食丸后才能再次进食。
* 嗉囊（crop）：WIP，部分鸟类无嗉囊。
...
* 羽迹（feather tract）：同羽区。
* 羽区（pteryla）：鸟类皮肤上着生羽毛的区域。
* 羽小枝（barbule）：
* 羽衣（plumage）：
* 羽枝（barb）：构成羽片的一系列结构。
